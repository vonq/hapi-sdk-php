...
```php
function createContract(string $xCustomerId, PostContractModel $body): CreateContractResponseModel
```
...
$body_channelId = 30.52;
$body_credentials = HAPILib\ApiHelper::deserialize('{"key1":"val1","key2":"val2"}');
$body = new Models\PostContractModel(
    $body_channelId,
    $body_credentials
);
...
| Parameter | Type | Tags | Description |
|  --- | --- | --- | --- |
| `channelId` | `float` | Template, Required | channel_id (number, required) |
...
$result = $contractsController->listAutocompleteValues($channelId, $postingRequirementName, $body);
```
...
$result = $contractsController->listChannels(null, null, null, $acceptLanguage);
```
...
$result = $contractsController->listContracts($xCustomerId);
```
...
```php
function retrieveChannel(string $channelId, ?string $acceptLanguage = null): ChannelModel
```
...
$result = $contractsController->retrieveChannel($channelId, $acceptLanguage);
```
...
```php
function retrieveContract(string $contractId, string $xCustomerId): ContractModel
```
...
$result = $contractsController->retrieveMultipleContracts($contractsIds);
```
